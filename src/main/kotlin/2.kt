...
object `2` : Day {
    private data class Game(
        ...
        companion object {
            ...
            fun String.toGame() =
                Game(
                    id = gameRegex.find(this)!!.value.toInt(),
                    rounds = this.split(";").map { it.toRound() },
                )
        }
    }
    ...
    private data class Round(
        ...
        companion object {
            ...
            fun String.toRound() =
                Round(
                    redRegex.find(this)?.value?.toInt() ?: 0,
                    greenRegex.find(this)?.value?.toInt() ?: 0,
                    blueRegex.find(this)?.value?.toInt() ?: 0,
                )
        }
    }
    ...
    override fun secondSolution(input: List<String>): String {
        return splitInput().map {
            ...
            val maxRed = it.rounds.maxBy { it.red }.red
            val maxGreen = it.rounds.maxBy { it.green }.green
            val maxBlue = it.rounds.maxBy { it.blue }.blue
            BigDecimal(maxRed).times(BigDecimal(maxGreen)).times(BigDecimal(maxBlue))
        }.toPlainString()
    }
