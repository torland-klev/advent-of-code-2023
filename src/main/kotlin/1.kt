...
object `1` : Day {
    override fun firstSolution(input: List<String>) =
        input.sumOf { line ->
            line.filter {
                    c ->
                c.isDigit()
            }.let { number -> "${number.first()}${number.last()}" }.toInt()
        }.toString()

    override fun secondSolution(input: List<String>) =
        input.sumOf { line ->
    ...
    /**
     * I gave this a solid attempt, but I cannot seem to find the missing case.
     * If you can find a test that defeats this, please let me know.
     */
    fun secondSolutionIncomplete(input: List<String>) =
        input.sumOf { line ->
            replaceAllTextWithNumber(line).filter {
                    c ->
                c.isDigit()
            }.let { number -> "${number.first()}${number.last()}" }.toInt()
        }.toString()

    private fun replaceAllTextWithNumber(
        text: String,
        substringStart: Int = 0,
        substringEnd: Int = 2,
    ): String {
        val shortestWordInList = itemsToFind.minOfOrNull { it.length } ?: 1
        val longestWordInList = itemsToFind.maxOfOrNull { it.length } ?: 1

        return if (longestWordInList < substringEnd - substringStart || text.length < substringEnd && substringStart - shortestWordInList <= text.length) {
            val newStartIndex = substringStart + 1
            replaceAllTextWithNumber(text, newStartIndex, newStartIndex + shortestWordInList)
        } else if (text.length >= substringEnd && text.filterNot { it.isDigit() }.length > shortestWordInList) {
            val preDigits = text.substring(0, substringStart).filter { it.isDigit() }
            val currentText = text.substring(substringStart, substringEnd)
            val remainderText = text.substring(substringStart + 1)
            val match = currentText.findMatch()
            if (match != null) {
                replaceAllTextWithNumber(preDigits + match + remainderText, substringStart = 1, substringEnd = 1 + shortestWordInList)
            } else {
                replaceAllTextWithNumber(text, substringStart, substringEnd + 1)
            }
        } else {
            text
        }
    ...
    private fun String.replaceDigitStringWithDigitInt() =
        this
            .replace("one", "1")
            .replace("two", "2")
            .replace("three", "3")
    ...
    private val itemsToFind = listOf("one", "two", "three", "four", "five", "six", "seven", "eight", "nine")
