        ...
        input.forEachIndexed { rowI, it ->
            ...
            while (i < it.length) {
                if (it[i].isDigit()) {
                    ...
                    var numberStop = i
                    var lookAheadI = i + 1
                    while (lookAheadI < it.length && it[lookAheadI].isDigit()) {
                        numberStop = lookAheadI
                        lookAheadI++
                    }
                    val hasSymbolAbove = rowI > 0 && input[rowI - 1].substring((if (numberStart > 0) numberStart - 1 else 0)..(if (numberStop < input[rowI - 1].length - 1) numberStop + 1 else numberStop)).contains(Regex("[^0-9.]"))
