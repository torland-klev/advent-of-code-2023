...
object `5` : Day {
    override fun firstSolution(input: List<String>): String {
        ...
        return almanac.seeds.minOfOrNull { almanac.getLocationForSeed(it) }.toString()
    }
    ...
    override fun secondSolution(input: List<String>): String {
        ...
        val ranges =
            almanac.seeds.mapIndexed { index, seed ->
                if (index % 2 == 0) {
                    (seed..seed + almanac.seeds[index + 1])
                } else {
                    null
                }
            }.filterNotNull()
        ...
        ranges.forEach {
            it.forEach {
                val loc = almanac.getLocationForSeed(it)
                if (lowestLocation == 0L) {
                    lowestLocation = loc
                } else if (loc < lowestLocation) {
                    lowestLocation = loc
                }
            }
        }
    ...
    data class AlmanacMap(
        ...
        companion object {
            fun create(
                ...
                from: String,
                to: String?,
            ) = input.joinToString("\n").substringAfter(from).let { if (to != null) it.substringBefore(to) else it }.split("\n").filter {
                it.isNotBlank()
            }.map {
                val split = it.split(" ")
                AlmanacMap(
            ...
            fun destinationForSource(
                maps: List<AlmanacMap>,
                source: Long,
            ) = maps.firstOrNull {
                source in it.sourceRangeStart..it.sourceRangeStart + it.rangeLength
            }?.destinationForSource(source) ?: source
        }

        fun destinationForSource(source: Long) =
    ...
    data class Almanac(
        val seeds: List<Long>,
        val seedToSoilMap: List<AlmanacMap>,
        val soilToFertilizerMap: List<AlmanacMap>,
        val fertilizerToWaterMap: List<AlmanacMap>,
        val waterToLightMap: List<AlmanacMap>,
        val lightToTemperatureMap: List<AlmanacMap>,
        ...
        companion object {
            fun create(input: List<String>) =
                Almanac(
                    seeds = input[0].substringAfter(": ").split(" ").filter { it.isNotBlank() }.map { it.toLong() },
                    seedToSoilMap = AlmanacMap.create(input, "seed-to-soil map:", "soil-to-fertilizer map:"),
                    soilToFertilizerMap = AlmanacMap.create(input, "soil-to-fertilizer map:", "fertilizer-to-water map:"),
                    fertilizerToWaterMap = AlmanacMap.create(input, "fertilizer-to-water map:", "water-to-light map:"),
                    waterToLightMap = AlmanacMap.create(input, "water-to-light map:", "light-to-temperature map:"),
                    lightToTemperatureMap = AlmanacMap.create(input, "light-to-temperature map:", "temperature-to-humidity map:"),
                    temperatureToHumidityMap = AlmanacMap.create(input, "temperature-to-humidity map:", "humidity-to-location map:"),
                    humidityToLocationMap = AlmanacMap.create(input, "humidity-to-location map:", null),
                )
        }

        private fun getSoilForSeed(seed: Long) = AlmanacMap.destinationForSource(seedToSoilMap, seed)

        private fun getFertilizerForSoil(soil: Long) = AlmanacMap.destinationForSource(soilToFertilizerMap, soil)

        private fun getWaterForFertilizer(fertilizer: Long) = AlmanacMap.destinationForSource(fertilizerToWaterMap, fertilizer)

        private fun getLightForWater(water: Long) = AlmanacMap.destinationForSource(waterToLightMap, water)

        private fun getTemperatureForLight(light: Long) = AlmanacMap.destinationForSource(lightToTemperatureMap, light)

        private fun getHumidityForTemperature(temperature: Long) = AlmanacMap.destinationForSource(temperatureToHumidityMap, temperature)

        private fun getLocationForHumidity(humidity: Long) = AlmanacMap.destinationForSource(humidityToLocationMap, humidity)

        fun getLocationForSeed(seed: Long) =
            getLocationForHumidity(
                getHumidityForTemperature(
                    getTemperatureForLight(
                        getLightForWater(
                            getWaterForFertilizer(
                                getFertilizerForSoil(
                                    getSoilForSeed(seed),
                                ),
                            ),
                        ),
